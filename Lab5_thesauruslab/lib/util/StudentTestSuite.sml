  ...
  struct
    ...
    fun fromRefsol (f, fRef, cmp) : ('a, 'b) checker =
      let
        ...
        let val des = Result.run fRef input
          in (Result.compare cmp (result, des),des) end
      in
        (f, ver)
      end
    ...
    fun fromVerifier (f, ver) : ('a,'b) checker = (f, (fn i =>(ver i,Result.Exn VerificationFailed)))
